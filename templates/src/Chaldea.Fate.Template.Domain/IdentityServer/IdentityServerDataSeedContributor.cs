...
namespace Chaldea.Fate.Template.IdentityServer
{
    public class IdentityServerDataSeedContributor : IDataSeedContributor, ITransientDependency
    {
        ...
        {
            await CreateApiScopeAsync("Template");
        }
        ...
        private async Task CreateApiResourcesAsync()
        {
            ...
            await CreateApiResourceAsync("Template", commonApiUserClaims);
        }
        ...
        private async Task CreateClientsAsync()
        {
            ...
            if (!webClientId.IsNullOrWhiteSpace())
            {
                ...
                /* Template_Web client is only needed if you created a tiered
            ...
            // Blazor Client
            var blazorClientId = configurationSection["Template_Blazor:ClientId"];
            if (!blazorClientId.IsNullOrWhiteSpace())
            {
                var blazorRootUrl = configurationSection["Template_Blazor:RootUrl"].TrimEnd('/');

                await CreateClientAsync(
                    name: blazorClientId,
                    scopes: commonScopes,
                    grantTypes: new[] { "authorization_code" },
                    secret: configurationSection["Template_Blazor:ClientSecret"]?.Sha256(),
                    requireClientSecret: false,
